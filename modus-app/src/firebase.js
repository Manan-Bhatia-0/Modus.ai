...
export const editProfile = async (name, gender, age, birthday) => {
  await db
    .collection("users")
...
export const getrecommendedMHResources = async () => {
  ...
  const querySnapshot = await getDocs(
    collection(db.collection("mentalHealthResources").doc(level), "Resources")
  );
...
export const MoodAnalysisFuncs = async () => {
  ...
  if (return_lst == null) {
    return [
      {
        "Guided Meditation":
          "https://mindfullycity.com/free-guided-meditation-resources-for-difficult-times/",
      },
      "Male",
    ];
  }
